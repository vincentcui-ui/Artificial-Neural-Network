...
class PyTorchANN(nn.Module):
    ...
    def report_my_judgement(self) -> str:
        ...
        # >>> YOUR CODE HERE >>>
        if len(self.train_val_metrics["train_losses"]) == 0 or len(self.train_val_metrics["val_losses"]) == 0:
            raise ValueError("No training/validation metrics available to make a judgement.")
        
        train_loss = self.train_val_metrics["train_losses"][-1]
        val_loss = self.train_val_metrics["val_losses"][-1]
